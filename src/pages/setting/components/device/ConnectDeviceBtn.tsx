...
/**
 * 디바이스 미연결 시 표시하는 골드 그라데이션 연결 버튼.
 ...
 */
const ConnectDeviceBtn = ({ onClick }: ConnectDeviceBtnPropTypes) => {
  return (
    <button
      type="button"
      onClick={onClick}
      className="flex w-full items-center justify-center gap-sm rounded-lg py-sm tag-glass-effect"
      style={{
        background:
          'linear-gradient(0deg, rgba(242, 178, 4, 0.70) 0%, rgba(255, 226, 110, 0.70) 100%), rgba(255, 209, 66, 0.20)',
      }}
    >
      <span className="heading-lg-semibold text-neutral-100">
        디바이스 연결하기
      </span>
      <FontAwesomeIcon
        icon={faMicrochip}
