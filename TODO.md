...
- [x] 알림 설정 동의 안 했는데 설정 페이지에서 앱 푸시 알림이 디폴트로 켜져있음 -> 온보딩과 상태 연결,,
- [x] 설정에서 알림 권한 요청 버튼 지우기
...
- [] 로그아웃 추가 (지금 api는 없는데 화면에 띄우기는 해야 할듯 버튼)
- [] 탈퇴하기 추가 (구글 로그인 이제 계정이...없다... 온보딩을 봐야 하는데;)

- [] 설정에서 알림 토글이 상태를 불러올 때 왼쪽에서 오른쪽으로 이동함 (푸시 토글이,, 들어가자마자 보이면 좋을텐데?)
